...
const askAIEthical = async (text) => {
  try {
    ...
    const question = 'You will recieve a job posting from the user. You need to read it and answer 3 questions. 1. Is this job ethical? Your answer should be true or false. A job is ethical if it benefits the environment (alternative energy, converting existing practices to green ones, etc), animal or human rights (e.g. healthcare, education, social services, etc.), is a non-profit, political organizing, or similar ethical efforts. Avoid considering jobs that contribute to general societal functioning without a clear ethical focus (e.g., exercise/fitness, cyber security, open source software, hr/recruiting, student loans, dieting, etc). Only consider these ethical if they have a clear ethical application (e.g. an open source software company that distributes access to software to underprivelidged communities). Additionally do not consider how the employer treats its employees or conducts business internally as a factor when determining whether the job is ethical (for example if an employer promotes DEI in their hiring process, that\'s not ethical work. However if they have a product that promotes DEI in other businesses, that is). 2. What is the reason for your answer to question 1? This should be short (ex: improves vetrinarian software, increases green spending habbits, etc). 3. Does this job REQUIRE backend experience? Note the difference between frontend and backend. Technologies like React and Javascript are frontend, while technolgies like Go, Python, Kafka, Rust, Ruby, etc are backend. You should only label this true if it is clear the job REQUIRES backend experience. If it simply mentions it, or you are unsure, default to false. Your response must be in valid JSON format. Here is an example response: {"isEthical": false, "reason": "generic business software", "requiresBackend": true}';
